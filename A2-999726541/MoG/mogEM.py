...
def mogEM(x, K, iters, randConst=1.0, minVary=0.0, init_kmeans=False):
    """
...
def q2():
    ...
    #------------------- Add your code here ---------------------
    p1, mu1, vary1, logLH1 = mogEM(inputs_train, K, iters=iters, minVary=minVary, randConst=randConst)
    print "Q4.2"
    ShowMeans(mu1,number=0)
    ShowMeans(vary1,number=1)

    print "Q4.3"
    p2, mu2, vary2, logLH2 = mogEM(inputs_train, K, iters=iters, minVary=minVary, randConst=randConst, init_kmeans=True)
    ShowMeans(mu2, number=0)
    ShowMeans(vary2, number=1)
...
def q4():
    ...
    for t in xrange(T):
        ...
        #-------------------- Add your code here ------------------------------
        p1, mu1, vary1, logLH1 = mogEM(x_train_anger, K, iters=iters, minVary=minVary, randConst=randConst,init_kmeans=True)
        p4, mu4, vary4, logLH4 = mogEM(x_train_happy, K, iters=iters, minVary=minVary, randConst=randConst,init_kmeans=True)
        #------------------- Answers ---------------------
        ...
        #-------------------- Add your code here ------------------------------
        ln_p1_train = mogLogLikelihood(p1, mu1, vary1, x_train)
        ln_p1_valid = mogLogLikelihood(p1, mu1, vary1, x_valid)
        ln_p1_test = mogLogLikelihood(p1, mu1, vary1, x_test)

        ln_p4_train = mogLogLikelihood(p4, mu4, vary4, x_train)
        ln_p4_valid = mogLogLikelihood(p4, mu4, vary4, x_valid)
        ln_p4_test = mogLogLikelihood(p4, mu4, vary4, x_test)
        errorTrain[t] = error_rate(ln_p1_train, ln_p4_train, y_train)
        errorValidation[t] = error_rate(ln_p1_valid, ln_p4_valid, y_valid)
        errorTest[t] = error_rate(ln_p1_test, ln_p4_test, y_test)
...
def error_rate(prob_d1, prob_d4, targets):
    errors = 0
    for i,t in enumerate(targets.T):
        if ((prob_d1[i] >= prob_d4[i]) and t == 1) or ((prob_d4[i] > prob_d1[i]) and t == 0):
            errors += 1
