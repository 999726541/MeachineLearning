    ...
    for t in xrange(hyperparameters['num_iterations']):
        ...
        print ("ITERATION:{:4d}  TRAIN NLOGL:{:4.2f}  TRAIN CE:{:.6f} "
               "TRAIN FRAC:{:2.2f}  VALID CE:{:.6f}  VALID FRAC:{:2.2f}").format(
                   t+1, f[0] / N, cross_entropy_train, frac_correct_train*100,
...
if __name__ == '__main__':
    ...
    logging /= num_runs
    ce_train = [i[1] for i in logging]
    ce_valid = [i[3] for i in logging]
    iteration = [i for i in range(len(logging))]
    # testing...
    weights = weights_all[0]
    test_data,test_label = load_test()
    test_result = logistic_predict(weights,test_data)

    test_result = np.array(test_result>=0.5).astype(np.int)
    print 1-float(sum(abs((test_result-test_label))))/float(len(test_label))
    print test_result.T
    print test_label.T
    # TODO generate plots

    plt.plot(ce_train,label ='pp')
    plt.plot(ce_valid,label = 'ww')
    plt.xlabel('Iteratiom')
    plt.ylabel('CE')
